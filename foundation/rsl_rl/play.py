...
class ActorWrapper(torch.nn.Module):
    def __init__(self, policy, device):
        super().__init__()
        self.actor_cnn = policy.actor_cnn
        self.actor_rnn = policy.actor_memory.rnn
...
#     model = ActorWrapper(policy=policy_nn, device=agent_cfg.device)
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    obs, _ = env.get_observations()
    print(f"[INFO] First frame observations: {obs.cpu().numpy()}")  # 转换为 numpy 格式以便打印
    obs_save_path = os.path.join(log_root_path, "observations")
    os.makedirs(obs_save_path, exist_ok=True)  # 确保目录存在
    obs_file = os.path.join(obs_save_path, "first_frame_obs.npy")
    np.save(obs_file, obs.cpu().numpy())  # 保存为 .npy 文件
    print(f"[INFO] First frame observations saved to: {obs_file}")
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            # agent stepping
            actions = policy(obs)
            # print actions in real-time
            print(f"[INFO] Timestep {timestep}: Actions: {actions.cpu().numpy()}")  # 转换为 numpy 格式以便打印
            # env stepping
            obs, _, _, _ = env.step(actions)
