...
def main():
    # 1. 设置命令行参数解析
    parser = argparse.ArgumentParser(description="Plot and Save RNN hidden states from multiple trajectory NPZ files.")
    ...
    for i, (h_states, time_arr) in enumerate(zip(all_h_states, all_time_arrs)):
        ...
        if mark_start <= mark_end:
            ...
            if len(marker_indices) > 0:
                ...
                # ==========================================
                # [核心修正] 提取当前轨迹的 Colormap 和 Normalizer
                # ==========================================
                cmap = lc.get_cmap()
                norm = lc.norm
                ...
                for idx in marker_indices:
                    ...
                    ax.annotate(f"{t_val:.2f}s", (x_plot[idx], y_plot[idx]), 
                                ...
                                color=label_color, # <-- [修正] 使用计算出的颜色
                                # 文字底色依然保留白色半透明 bbox，确保文字清晰可见
    ...
    ax.autoscale() 
    ax.set_title(f'RNN Hidden Space Dynamics Manifold (PCA 2D Projection)', fontsize=15, fontweight='bold')
    ax.set_xlabel(f'Global Principal Component 1 ({pca.explained_variance_ratio_[0]*100:.1f}% Variance)')
    ax.set_ylabel(f'Global Principal Component 2 ({pca.explained_variance_ratio_[1]*100:.1f}% Variance)')
    ...
    file_name = f"hidden_state_manifold_{timestamp}.png"
