...
class Quadrotor:
    """
    Complete PX4-style quadrotor controller combining attitude and rate control.
    Matches the interface of controller.py's Quadrotor class but with PX4 logic.
    Includes motor dynamics and control allocation.
    """
    ...
    def __init__(self,
                 ...
                 viz_port: int = 8000):
        """
        Initialize complete controller (compatible with original controller.py interface).

        Args:
            dynamics: QuadrotorDynamics instance (if None, creates default)
            num_envs: Number of parallel environments
            device: PyTorch device
            Krp_ang: Attitude P gains [roll, pitch, yaw] (maps to att_p_gain)
            Kdrp_ang: Not used in PX4 (PX4 uses rate control instead)
            Kinv_ang_vel_tau: Rate P gains [roll, pitch, yaw] (maps to rate_p_gain)
            att_yaw_weight: Yaw priority weight [0,1]
            debug_viz: Enable debug visualization (not implemented)
            viz_port: Visualization port (not implemented)
        """
        # Initialize dynamics model
        self.dynamics = dynamics if dynamics is not None else QuadrotorDynamics(
            num_envs=num_envs,
            device=device
        )
        self.device = device
        ...
        # Convert original controller parameters to PX4 parameters
        # Krp_ang -> attitude P gains
        if Krp_ang is None:
            att_p_gain = torch.tensor([6.5, 6.5, 2.8], device=device)
        else:
            if isinstance(Krp_ang, (list, tuple)):
                att_p_gain = torch.tensor(Krp_ang, device=device)
            elif isinstance(Krp_ang, torch.Tensor):
                att_p_gain = Krp_ang.to(device)
                if att_p_gain.dim() == 2:  # If batched, take first
                    att_p_gain = att_p_gain[0]
            else:
                att_p_gain = torch.tensor([6.5, 6.5, 2.8], device=device)

        # Kinv_ang_vel_tau -> rate P gains
        # Note: Original uses 1/tau as P gain, PX4 uses direct P gain
        # We scale down by ~40x to match typical PX4 values
        if Kinv_ang_vel_tau is None:
            rate_p_gain = torch.tensor([0.15, 0.15, 0.18], device=device)
        else:
            if isinstance(Kinv_ang_vel_tau, (list, tuple)):
                rate_p_gain = torch.tensor(Kinv_ang_vel_tau, device=device) / 40.0
            elif isinstance(Kinv_ang_vel_tau, torch.Tensor):
                rate_p_gain = Kinv_ang_vel_tau.to(device)
                if rate_p_gain.dim() == 2:  # If batched, take first
                    rate_p_gain = rate_p_gain[0]
                rate_p_gain = rate_p_gain / 40.0
            else:
                rate_p_gain = torch.tensor([0.15, 0.15, 0.18], device=device)
        ...
        self.attitude_control = AttitudeControl(
            ...
            rate_limit=torch.tensor([3.0, 3.0, 1.5], device=device)
        )
        ...
        print(f"[PX4 Controller] Initialized with {self.num_envs} environments")
        print(f"  Mass: {self.dynamics.mass_[0]:.4f} kg")
        print(f"  Max thrust: {self.dynamics.thrust_max_[0]:.4f} N")
    ...
    def compute_control(self,
                       state: torch.Tensor,
                       cmd: torch.Tensor,
                       dt: float = 0.005) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Main control loop with motor dynamics and control allocation.
        ...
        Args:
            ...
            cmd: (num_envs, 4) normalized commands:
                 [roll_cmd, pitch_cmd, yaw_cmd, thrust_cmd] in [-1, 1] or [0, 1]
            dt: timestep
        ...
        # Build desired attitude quaternion from commands
        # Map commands to physical angles (±45deg for roll/pitch, ±90deg for yaw)
        roll_des = cmd[:, 0] * (math.pi / 4)
        pitch_des = cmd[:, 1] * (math.pi / 4)
        yaw_des = cmd[:, 2] * (math.pi / 2)
        thrust_cmd = cmd[:, 3]  # Normalized [0, 1]

        # Map thrust command to physical thrust in Newtons
        ...
        # === Attitude control: quat -> rate_sp ===
        rate_sp = self.attitude_control.update(q_cur)

        # === Rate control: rate_sp -> torque ===
        # Compute angular acceleration (finite difference or from state)
        # For now, approximate as zero (you can add proper differentiation)
        angular_accel = torch.zeros_like(omega_body)
        ...
        # Based on controller.py baseline-plus-differential allocation method
        ...
        # Step 3: Add gyroscopic compensation to desired torque
        # PX4 rate controller outputs normalized torque, we need to convert to physical torque
        Jw = torch.einsum('nij,nj->ni', self.dynamics.J_, omega_body)
        gyro = torch.cross(omega_body, Jw, dim=1)

        # Scale rate controller output by inertia
        Jtau = torch.einsum('nij,nj->ni', self.dynamics.J_, tau_des)
        tau_des_physical = Jtau + gyro

        # Step 4: Compute differential thrust for torque control
        ...
        # Step 6: Combine baseline and differential thrust
        motor_thrusts = T0.unsqueeze(1) + delta_T  # Shape: [batch, 4]

        # Step 7: Enforce non-negative thrust
        ...
        # Step 8: Limit per-motor maximum thrust using uniform scaling
        ...
        # Step 9: Reconstruct resulting force & torque from actual motor thrusts
        ...
        # Check if any motor is saturated
        ...
        # Determine which axes are saturated based on allocation matrix
        # This is a simplified version - full implementation would check sign consistency
