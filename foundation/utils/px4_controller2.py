...
class PX4QuadrotorController:
    ...
    def __init__(self,
        ...
        # Compute normalized pseudo-inverse matrix (PX4-style)
        # This normalizes the pseudo-inverse so normalized torque [-1,1] maps to normalized actuators
        self.alloc_matrix_pinv_normalized_, self.control_allocation_scale_ = self._compute_normalized_pinv()

        # Flag to enable/disable normalization (PX4-style control allocation)
        self.use_px4_allocation = False  # Default: disabled (direct physical torque)
        ...
        print(f"  Control allocation scale [Roll, Pitch, Yaw]: {self.control_allocation_scale_[0, :3]}")
        print(f"  PX4-style allocation: {'ENABLED' if self.use_px4_allocation else 'DISABLED'}")
    ...
    def enable_px4_allocation(self, enable: bool = True):
        """
        Enable or disable PX4-style normalized control allocation.

        When enabled, follows PX4's approach:
        1. Rate controller outputs normalized torque [-1, 1]
        2. Pseudo-inverse matrix is normalized (divided by max values)
        3. Allocation produces normalized actuator commands
        4. Motor thrust differentials stay within reasonable bounds

        When disabled, uses direct physical torque calculation (may cause large motor differentials).

        Args:
            enable: True to enable PX4-style allocation, False for direct physical allocation
        """
        self.use_px4_allocation = enable
        print(f"[PX4 Controller] PX4-style allocation: {'ENABLED' if enable else 'DISABLED'}")
        if enable:
            print(f"  Normalized pseudo-inverse prevents excessive motor thrust differentials")
            print(f"  Control allocation scale: {self.control_allocation_scale_[0, :3]}")

    def _compute_normalized_pinv(self) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Compute normalized pseudo-inverse allocation matrix following PX4's approach.

        PX4 normalizes the pseudo-inverse matrix (_mix) by dividing each control axis column
        by its maximum absolute value. This ensures that normalized torque commands [-1, 1]
        map to reasonable actuator differentials without excessive values.

        The normalization scale is stored separately to enable reconstruction of
        allocated control for verification: allocated = (effectiveness @ actuators) * scale

        Returns:
            tuple: (normalized_pinv, control_allocation_scale)
                - normalized_pinv: (num_envs, 4, 4) normalized pseudo-inverse matrix
                - control_allocation_scale: (num_envs, 6) scaling factors for each axis
        """
        pinv = self.alloc_matrix_pinv_.clone()
        num_envs = pinv.shape[0]

        # Initialize control allocation scale (6 axes: roll, pitch, yaw, thrust_x, thrust_y, thrust_z)
        control_allocation_scale = torch.ones(num_envs, 6, device=self.device)

        # For each control axis (columns 1-3: roll, pitch, yaw in pseudo-inverse)
        for axis_idx in range(1, 4):  # Roll=1, Pitch=2, Yaw=3
            # Find maximum absolute value across all motors (rows) for this control axis
            max_vals = torch.max(torch.abs(pinv[:, :, axis_idx]), dim=1)[0]  # shape: (num_envs,)

            # Avoid division by zero
            max_vals = torch.clamp(max_vals, min=1e-6)

            # Store the scale factor (this is what we divide by)
            control_allocation_scale[:, axis_idx - 1] = max_vals

            # Normalize the column: divide by max value so range becomes [-1, 1]
            pinv[:, :, axis_idx] = pinv[:, :, axis_idx] / max_vals.unsqueeze(1)

        return pinv, control_allocation_scale
    ...
    def compute_control(self,
        ...
        torque_only_wrench = torch.cat([
            ...
            tau_des  # Normalized torque from rate controller [-1, 1]
        ], dim=1)

        if self.use_px4_allocation:
            # PX4-style: Use normalized pseudo-inverse
            # tau_des is normalized [-1, 1], pinv is also normalized
            # Result: normalized actuator DIFFERENTIALS (not absolute values!)
            actuator_diff_normalized = torch.bmm(self.alloc_matrix_pinv_normalized_, torque_only_wrench.unsqueeze(-1)).squeeze(-1)

            # PX4's formula: actuator_sp = actuator_trim + pinv * control_sp
            # For motors, actuator_trim = hover_thrust (normalized, ~0.5)
            # We compute thrust differentials from the normalized actuator differentials
            # Assuming trim = 0 for differential calculation:
            # delta_T[N] = actuator_diff_normalized * thrust_max (since actuator ∈ [0,1])
            delta_T = actuator_diff_normalized * T_motor_max.unsqueeze(1)
        else:
            # Direct physical allocation (original, may cause excessive differentials)
        ...
        info = {
            ...
            'force_z': force[:, 2],                # Actual thrust force [N]
