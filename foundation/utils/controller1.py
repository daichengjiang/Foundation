...
from isaaclab.utils.math import euler_xyz_from_quat,quat_mul,axis_angle_from_quat,quat_from_euler_xyz,quat_inv, quat_rotate_inverse
# controller.py 文件顶部
def quat_multiply(q1, q2):
    ...
    Args:
        q1 (torch.Tensor): 第一个四元数，形状 (N, 4)
        q2 (torch.Tensor): 第二个四元数，形状 (N, 4)
        
    Returns:
        torch.Tensor: 乘积结果，形状 (N, 4)
    """
    # 按照 [w, x, y, z] 的顺序解包
...
class Quadrotor:
    ...
    def compute_control(self, state: torch.Tensor, cmd: torch.Tensor, dt: float = 0.005):
        ...
        roll_des     = cmd[:,0] * (math.pi/2)
        pitch_des    = cmd[:,1] * (math.pi/2)
        yaw_des      = cmd[:,2] * (math.pi/2)
        F_des        = cmd[:,3] * self.dynamics.thrust_max_

        q_cur = state[:, 3:7]
        q_des = quat_from_euler_xyz(roll_des, pitch_des, yaw_des)
        q_err = quat_mul(quat_inv(q_cur), q_des)
        axis_angle_err = axis_angle_from_quat(q_err)
        roll_err, pitch_err, yaw_err = axis_angle_err[:, 0], axis_angle_err[:, 1], axis_angle_err[:, 2]
        omega_cur = state[:, 10:13]
        omega_cur = quat_rotate_inverse(state[:, 3:7], omega_cur)



        # F_des = torch.full_like(cmd[:,3], 0.5) 
        ...
        # print("roll_rate_des:", roll_rate_des[0])
        # print("pitch_rate_des:", pitch_rate_des[0])
        # print("yaw_rate_des:", yaw_rate_des[0])
        # yaw_rate_des = torch.full_like(yaw_rate_des, 0.0)  # Disable yaw control for now
        # roll_rate_des = torch.full_like(roll_rate_des, 0.00)  # Disable yaw control for now
        # pitch_rate_des = torch.full_like(pitch_rate_des, 0.05)  # Disable yaw control for now

        # c_phi = torch.cos(roll_cur)
        # s_phi = torch.sin(roll_cur)
        # c_theta = torch.cos(pitch_cur)
        # s_theta = torch.sin(pitch_cur)

        # # 针对 "XYZ extrinsic" 的转换公式实现
        # p_des = roll_rate_des - s_theta * yaw_rate_des
        # q_des = c_phi * pitch_rate_des + s_phi * c_theta * yaw_rate_des
        # r_des = -s_phi * pitch_rate_des + c_phi * c_theta * yaw_rate_des
        # w_des = torch.stack([p_des, q_des, r_des], dim=1)
