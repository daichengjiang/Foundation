...
from e2e_drone.utils.controller import Quadrotor, QuadrotorDynamics
...
class QuadcopterEnvCfg(DirectRLEnvCfg):
    ...
    # controller_Kang = [15.0, 15.0, 20]  # Roll and pitch angle controller gains   #15 15 20
    # controller_Kdang = [0.8, 0.8, 1.2]                                            #0.8 0.8 1.2
    # controller_Kang_vel = [10.0, 10.0, 10.0]  # Roll, pitch, and yaw angular velocity controller gains
...
class QuadcopterEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        start.record()
        force, torque, ctrl_info = self._controller.compute_control(cur_state, self._actions, self.step_dt)
        # force, torque, ctrl_info = self._controller.compute_control(cur_state, mean_action, self.step_dt)
        end.record()
        torch.cuda.synchronize()

        # Store controller info for visualization (angles in radians)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        weights = torch.tensor([1.0, 2.0, 3.0, 1.0], device=self.device)
