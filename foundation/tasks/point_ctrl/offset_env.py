...
class QuadcopterDynamicsCfg:
    # 默认值 (Crazyflie 2.1) - 仅在没有 multi_teacher_params 时使用
    mass: float = 0.0282
...
# ----------------------------------------------------------------------
    # 将 offset_env.py 中的 _set_debug_vis_impl 替换为以下代码
    # ----------------------------------------------------------------------
    def _set_debug_vis_impl(self, debug_vis: bool):
        ...
        if debug_vis:
            ...
            # 2. Current Robot Position (蓝色球 - 机器人中心)
            ...
            if not hasattr(self, "traj_langevin_visualizer"):
                ...
                langevin_cfg.markers["sphere"].visual_material.diffuse_color = (0.0, 1.0, 0.0) # 绿色
                self.traj_langevin_visualizer = VisualizationMarkers(langevin_cfg)
            self.traj_langevin_visualizer.set_visibility(True)

            # 4. [新增/关键] Fixed/Null Trajectory (红色球 - 定点目标)
            # 因为 play_teacher_multi 强制为定点，所以主要是这里在起作用
            if not hasattr(self, 'traj_fixed_visualizer'):
                ...
                # 如果你也想让定点显示为绿色，把下面的 (1.0, 0.0, 0.0) 改为 (0.0, 1.0, 0.0)
                fixed_cfg.markers["sphere"].visual_material.diffuse_color = (1.0, 0.0, 0.0) # 红色
    ...
    # ----------------------------------------------------------------------
    # 将 offset_env.py 中的 _debug_vis_callback 替换为以下代码
    # ----------------------------------------------------------------------
    def _debug_vis_callback(self, event):
        ...
        invisible_pos[:, 2] = -100.0 # 藏到地底下
        
        # 2. 更新 Langevin 绿色球 (仅当任务是 Langevin 时显示)
        ...
        # 3. [新增/关键] 更新 Fixed 红色球 (当任务不是 Langevin 时显示)
        if hasattr(self, "traj_fixed_visualizer"):
            pos_red = invisible_pos.clone()
            mask = ~self._is_langevin_task # 取反，表示定点任务
            if mask.any():
