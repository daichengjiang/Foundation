...
class SAC:
    ...
    def _compute_action_scaling(env: VecEnv, device: str) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            Tuple of (upper_scaling, lower_scaling) tensors of shape (num_actions,).
        """
        # 四旋翼等无物理关节限制的飞行器直接返回默认缩放因子 [1.0, 1.0]
        num_actions = env.num_actions
        scaling_factors_upper = torch.ones(num_actions, device=device)
        scaling_factors_lower = torch.ones(num_actions, device=device)
        
        print("SAC: Quadrotor floating-base detected, using standard action scaling [1.0, 1.0].")
        return scaling_factors_upper, scaling_factors_lower
