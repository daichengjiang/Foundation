...
class ActorCriticRNN(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # self.actor_memory = Memory(depth_cnn_out_dim+state_hidden_dims[-1], type=rnn_type, num_layers=rnn_num_layers, hidden_size=rnn_hidden_dim)
        # self.critic_memory = Memory(depth_cnn_out_dim+state_hidden_dims[-1], type=rnn_type, num_layers=rnn_num_layers, hidden_size=rnn_hidden_dim)

        self.actor_memory = Memory(depth_cnn_out_dim+self.obs_size, type=rnn_type, num_layers=rnn_num_layers, hidden_size=rnn_hidden_dim)
        self.critic_memory = Memory(depth_cnn_out_dim+self.obs_size, type=rnn_type, num_layers=rnn_num_layers, hidden_size=rnn_hidden_dim)
    ...
    def evaluate(self, critic_observations, masks=None, hidden_states=None, **kwargs):
        ...
        if masks is not None:
            ...
            features = torch.zeros(max_seq_len, num_traj, fused_features.shape[-1], device=unpadded_obs.device)
            features[masks] = fused_features
            features = self.critic_memory(features, masks, hidden_states).squeeze(0)
        else:
            features = fused_features
            features = self.critic_memory(features, masks, hidden_states).squeeze(0)
